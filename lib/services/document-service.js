...
function normalizeCsvRows(rows) {
  logDebug("documents", "Normalizing CSV rows.", {
...
export async function saveCsvDocuments(file) {
  ...
  const { headers, rows } = parseDocumentsCsv(csvText);
  logDebug("documents", "Parsed CSV upload.", {
  ...
  logDebug("documents", "Saved CSV upload to database.", {
